...
- **MCP write tools default to OFF** (`package.json` schema, `src/types.ts:413`, `src/extension.ts:1243`). `baton.allowMcpWriteAccess` was `true` by default; it is now `false`. The MCP server still exposes the full read surface out of the box (`memory-search`, `memory-recent`, `memory-timeline`, `memory-get`) — but `memory-store`, `memory-delete`, `memory-correct`, `memory-retract`, and `memory-supersede` require an opt-in flip. This matches MCP's own human-in-the-loop guidance and the enterprise-safe defaults the v1.6.0 reviewer asked for under their "MCP write tools need stronger guardrails" item. **Breaking change for users who relied on the previous default**: if you want write tools, set `baton.allowMcpWriteAccess: true` in your settings. (Enterprise mode continues to force this off regardless of the user setting.)
...
- **Encrypted local storage** (`baton.storageEncryption: off | os-keychain | passphrase` with migration) — scoped for a v1.7 sprint.
...
**Production-grade memory upgrade.** Seven incremental phases that take Baton from "a session summarization tool" to a memory system developers can trust: every claim is grounded in evidence, every ranking is provenance-aware, every memory has a confidence that decays over time, and every search hop carries the full decision narrative. 136 new tests (153 → 289), 0 native deps, 0 schema migrations, fully backward-compatible with stores captured under 1.5.x.
...
- **Six new MCP tools** (`src/mcpServer.ts`) — every chat command above now has an MCP-tool equivalent so Cursor, Cline, Windsurf, Claude Desktop, and the Copilot CLI all get the full surface: `baton_entity`, `baton_snippets`, `baton_conflicts`, `baton_lineage`, `baton_explain`, `baton_graph`. All reuse the same pure-function helpers as the chat path — single source of truth.
...
### Tests (Phase 9)
+18 tests in `groundingPhase9.test.ts` covering: intent classifier (lookup / modify / investigate / mixed / unknown), path extraction, attach-token estimator, recommendation correctness per intent, MCP-unavailable degradation, large-file savings ratio, MCP catalog wiring (including the new `baton_route` entry), routing-primer presence/absence in startup context. Total suite is now 307/307 (was 289 in 1.6.0).
...
- **New button: `Persist, don't ask again`** — confirms the current snapshot **and** sets `baton.previewBeforePersist = false` at `ConfigurationTarget.Global`, so the modal will not show again on this machine.
- Modal `detail` now tells users where to re-enable the prompt: *Settings → `baton.previewBeforePersist`*. No magic — discoverable and reversible.
- After the choice lands, a 5-second status-bar message confirms what changed: `$(check) Baton: persist prompt disabled. Re-enable in Settings: baton.previewBeforePersist`.
- **Enterprise-mode interaction handled explicitly.** `types.ts:267` ORs `previewBeforePersist` with `enterpriseMode`, so disabling the former alone is insufficient when the latter is on. In that case the user now sees a follow-up warning with an `Open Settings` action that jumps straight to `baton.enterpriseMode` — no silent failure, no confused user wondering why the prompt is still there.
...
The release-consistency gate in **1.5.1** lives outside the extension — as an npm script + CI gate. A reviewer (correctly) pointed out that **Baton itself should be able to catch the same class of bug** at any time, not only at publish. Drift detection now ships as a first-class extension capability surfaced through every interface Baton already owns.
...
1. Downloading the matching version's `.vsix` + `baton-mem.vsix.sha256` from GitHub Releases
2. `shasum -a 256 -c baton-mem.vsix.sha256` to verify the bits
...
| Corporate-policy URL with HTTPS validation | `src/policySource.ts` + `baton.policySource` setting |
...
- **`baton.enterpriseMode`** — Strict privacy posture that disables terminal capture, raw snippets, MCP write tools, and team export.
- **`baton.captureCodeSnippets`**, **`baton.allowMcpWriteAccess`**, **`baton.allowTeamExport`**, **`baton.previewBeforePersist`** — New control surface for privacy-by-default workflows.
- **`baton.runPrivacyWizard`**, **`baton.auditMemory`**, **`baton.purgeMemory`** — Onboarding, audit, and purge commands for enterprise users.
...
- **`baton.idleTimeoutSeconds`** — New config (0–300s, default 30s) that triggers compression when editor is inactive, measured via editor and text-document activity hooks. Polls every 5s to stay lightweight.
- **`baton.customRedactionRules`** — New config array allowing users to define custom regex-based redaction rules (name, pattern, replacement, flags) for enterprise compliance modes (PCI-DSS, HIPAA, etc.). Rules compose after the built-in 26-rule set; invalid regex silently skipped.
- **`src/ciSeeder.ts`** — Headless CLI tool for pre-seeding memory from CI/CD pipelines. Reads JSON from stdin, applies redaction, merges into `~/.baton-mem/sessions.json`, deduplicates by content hash, and tags with seedLabel. Added to `package.json` bin as `baton-mem-ci-seed`.
...
- **`src/contextProvider.ts`** — `buildStartupContext()` now uses configurable session count (`baton.startupContextSessionCount`, default 5) instead of hardcoded 3.
...
- **`src/types.ts`**, **`package.json`** — New `baton.startupContextSessionCount` setting (1–20, default 5).
...
- **`src/timelinePanel.ts`** — Visual Memory Timeline WebviewPanel (`Baton: Open Visual Timeline`, shortcut `⌥⌘M`). Color-coded session cards by observation type, full-text search, branch filter, expandable detail on click.
...
- **`src/extension.ts`** — Keyboard shortcut `⌘⇧⌥S` / `Ctrl+Shift+Alt+S` wired to `baton.captureSnapshot`.
- **`src/extension.ts`** — `baton_search` and `baton_store` registered as VS Code agent-mode tool sets via `vscode.lm.registerTool`.
...
- **`src/test/mcpServer.test.ts`** — TOOLS count assertion updated from 4 → 6 (added `baton_store`, `baton_delete`).
...
- **`README.md`** — Agent mode tools table lists `baton_search` and `baton_store` (registered as languageModelTools).
...
### Fixed
- **`src/mcpServer.ts`** — `baton_timeline` now returns most-recent activity first (`endTime` descending) so MCP clients show the newest context by default.
...
- **`src/sessionsView.ts`** — Quick-filter bar on the sidebar tree: scope (workspace/repo/all) · observation type · tag · last-N-days · free-text. Active filter is shown as a clickable chip in the header that clears on click. Wired through new commands `baton.filterSessions` and `baton.clearFilter`.
- **`src/markdownExport.ts`** — Diff-friendly session markdown exporter. Stable byte-identical output (sorted arrays, ISO timestamps, deterministic ordering) so committing exports into a repo produces clean diffs. Exposed via `baton.exportSessionMarkdown`.
- **`src/eval.ts`** — Lightweight retrieval evaluation harness (recall@k + MRR) comparing keyword-only vs hybrid vs hybrid+freshness configurations against a self-generated query set. Wired through `baton.runEval`.
- **`src/test/validator.test.ts`**, **`src/test/repoScope.test.ts`**, **`src/test/markdownExport.test.ts`** — Unit tests for the new modules.
- **`baton.scope`** (`user` | `workspace` | `repo`, default `user`) — picks the retrieval scope.
- **`baton.validateAgainstCodebase`** (default `true`) — toggles the validator.
- **`baton.freshnessFloor`** (0-1, default `0.25`) — minimum fraction of `keyFiles` that must still exist for a memory to survive validation.
- **`baton.githubCompatibleMode`** (default `false`) — mirrors GitHub agentic memory's contract: 28-day retention + repo scope (overrides the two settings above when enabled).
...
### Fixed
- **`extension.ts`** — Wrong publisher ID `ghcp-plugin.baton-mem` → `itcredibl.baton-mem` in `showMcpInfo` command; MCP server path was always showing placeholder text instead of the real install location.
...
- **`mcpServer.ts`** — `baton_search` and `baton_recent` now accept `workspaceName` (case-insensitive substring) as an alternative to `workspaceId`; external MCP clients (Cursor, Claude Desktop) can filter by workspace without knowing the full URI.
...
- **`contextStore.ts`** — `restoreFromBackup()` now applies redaction (previously bypassed unlike `importFromJson`). `importFromJson()` now validates session IDs as UUIDs, silently skipping malformed entries. `~/.baton-mem/sessions.json` is written with mode `0600`, its directory with `0700`.
...
### Fixed
- Removed all remaining `Oluseyi-Kofoworola` references from `README.md` and `docs/COMPARISON.md`; all links now point to `github.com/ITcredibl/baton-mem`.
...
- Git remote `origin` updated to `https://github.com/ITcredibl/baton-mem.git`.
...
- Extension first published to Marketplace under `itcredibl.baton-mem`.
...
### Added
- **Health alert threshold** (`baton.healthAlertThreshold`, default `30`): a warning
...
- **Workspace-scoped MCP queries**: `baton_search` and `baton_recent` now accept
...
- `Baton: Seed Azure Demo Sessions` command for demo/onboarding.
