...
│                                                                  │
│   ┌────────────────────────────┐   ┌──────────────────────────┐ │
│   │ ~/.ghcp-mem/sessions.json  │   │ VS Code globalState      │ │
...
- **B3** — Extension host ↔ `~/.ghcp-mem/sessions.json` (persistence)
...
| Compressed session record | `globalState` + `~/.ghcp-mem/sessions.json` | Medium (redacted summaries + key files) |
...
| Threat | Mitigation | Residual risk |
|---|---|---|
| **T6** Information disclosure — LM payload contains secrets despite first-pass redaction | LM input is the **already redacted** session data; a second redactor pass runs on the LM **output** too. Snippets are truncated to 200 chars and length-bounded before redaction. `ghcpMem.captureCodeSnippets=false` (enterprise mode) disables raw snippets entirely. | Medium-low — Copilot's terms of service govern Copilot data handling; GHCP-MEM cannot bind that behaviour. Documented in `SECURITY.md`. |
...
| **T8** Network exfil to non-Copilot endpoint | GHCP-MEM only calls `vscode.lm.sendRequest`. There is no `fetch`, `http`, or `net` call to a GHCP-MEM-owned backend. (Exception: `policySource` HTTPS GET, see B4.) Verifiable via `grep -nE "fetch\|http\\.\|https\\.\|net\\.\\|axios" src/`. | Low |

### B3 — Extension host ↔ `~/.ghcp-mem/sessions.json`

| Threat | Mitigation | Residual risk |
|---|---|---|
| **T9** Information disclosure — another local user reads the file | Directory is created with mode `0700`, file with mode `0600`. | **High** if attacker has root or shared-account access. Mitigation: full-disk encryption + per-user OS account; optional encrypted store gated behind `ghcpMem.encryptLocalStore` is on the 1.5.x roadmap. |
...
| Threat | Mitigation | Residual risk |
|---|---|---|
| **T15** Untrusted MCP client (e.g. Cursor / Cline / a malicious AI agent) issues a `ghcpMem_store` call with attacker-crafted payload | Input is validated against the tool's JSON schema; strings are length-bounded; `keyFiles` paths are normalised and rejected if they contain `..` or are absolute to another workspace. Redaction runs on every input field. `ghcpMem.allowMcpWriteAccess=false` (enterprise mode) disables write tools entirely. | Low to medium — write surface is narrow but exists. |
| **T16** `ghcpMem_search` returns memories from one workspace to a client running in a different workspace | MCP server filters by `workspaceId` / `workspaceName` when the client requests it. Without filter, all workspaces are searched (intended for cross-project recall). | Low — documented in README. |
...
These are **not** threats GHCP-MEM tries to defend against; users with these requirements should layer other controls:
...
- **Copilot subscription terms** — GHCP-MEM cannot constrain what GitHub does with the LM payload after it leaves the extension; the user's existing subscription terms govern that.
- **Network-level observation** between the extension host and Copilot's LM endpoint — this is the same trust as any other Copilot operation; no GHCP-MEM-specific risk added.
...
|---|---|---|
| **R1** Plaintext local store (T9) | Mode 0600 + OS perms only | `ghcpMem.encryptLocalStore` opt-in via `vscode.SecretStorage`-backed AES-256-GCM key — **1.5.x** |
...
# T8 — no GHCP-MEM-owned network calls
