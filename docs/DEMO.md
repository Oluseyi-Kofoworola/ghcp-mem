...
> [!NOTE]
> **Prereqs:** GHCP-MEM v1.7.0 installed (Marketplace search **GHCP-MEM**, or `code --install-extension ITcredibl.ghcp-mem`) and VS Code reloaded. Open any workspace. No Azure subscription needed — the Azure steps degrade gracefully if `az` isn't installed or signed in.
...
```mermaid
timeline
    title GHCP-MEM v1.7.0 — 6-minute live demo
    section 🩺 Health
...
<sub>**Demo script for GHCP-MEM v1.7.0** · 307 tests · zero native deps · zero ports · CI ubuntu × windows × node 20</sub>
